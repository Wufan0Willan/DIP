...
class RawAudioDatasetMix(FairseqDataset):
    def __init__(
        self,
        sample_rate,
        ...
        **mask_compute_kwargs,
        ...
        print("mixing_max_len:{0}".format(mixing_max_len))
        print("mixing_prob:{0}".format(mixing_prob))
    ...
    def collater(self, samples):
        ...
        if self.mixing_prob >= 0:
            collated_mix_audios, mix_list = self.mixing_collated_audios(collated_sources, target_size)
        #print("***************collated sources*******************")
        #print(collated_sources)
        input = {"source": collated_mix_audios}
        input["clean"] = collated_sources
        input["interference"] = mix_list 
    ...
    def mixing_collated_audios(self, source_list, audio_size):
        ...
        mix_list_1 = source_list[0].new_zeros(len(source_list), audio_size) 
        mix_list_2 = source_list[0].new_zeros(len(source_list), audio_size)
        B = source.shape[0]
        T = source.shape[1]
            ...
            if np.random.random() < self.mixing_prob:
                if self.mixing_noise and np.random.random() < self.mixing_noise_prob:
                    ...
                    for c in choices:
                        ...
                        s_end = np.random.randint(c_len, T + 1)
                        s_start = s_end - c_len
                        #print("c_start:{0} c_end:{1} s_start:{2} s_end:{3}".format(c_start, c_end, s_start, s_end))
                        ref_pow = np.mean(source_audio[i].numpy() ** 2)
                        noise_pow = np.mean(source_audio[c].numpy() ** 2)
                        if noise_pow == 0:
                            scale = 0
                        else:
                            snr_1 = np.random.uniform(-5, 5)
                            scale_1 = (ref_pow / (noise_pow * 10 ** (snr / 10))) ** 0.5
                            snr_2 = np.random.uniform(-5, 5)
                            scale_2 = (ref_pow / (noise_pow * 10 ** (snr / 10))) ** 0.5

                        source[i, s_start:s_end] += source_audio[c, c_start:c_end].clone() * scale
                        mix_list[i,  c_start:c_end] += source_audio[c, c_start:c_end].clone() * scale
                if self.normalize:
                    print("norm")
                    with torch.no_grad():
                        source[i] = F.layer_norm(source[i], source[i].shape)

        return source, mix_list
...
class FileAudioDatasetMix(RawAudioDatasetMix):
    def __init__(
        self,
        manifest_path,
        sample_rate,
...
class BinarizedAudioDataset(RawAudioDatasetMix):
    def __init__(
        self,
        data_dir,
        split,
