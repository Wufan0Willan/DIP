...
class RawAudioDatasetMMDC(FairseqDataset):
    def __init__(
        self,
        sample_rate,
    ...
    def collater(self, samples):
        ...
        source_domains = [s["domain"] for s in samples]
        source_clusters = [s["cluster"] for s in samples]
        #pdb.set_trace()
        ...
        #print("************************")
        input = {"source": collated_sources, "domain": source_domains, "cluster": source_clusters}
        #pdb.set_trace()
...
class FileAudioDatasetMMDC(RawAudioDatasetMMDC):
    def __init__(
        self,
        manifest_path,
        sample_rate,
        ...
        **mask_compute_kwargs,
        ...
        self.num_clusters = int(manifest_path.split(r".")[0].split('_')[3])
    ...
    def __getitem__(self, index):
        ...
        #pdb.set_trace()
        if domain_index == 0:
            cluster = self.source_labels[index]
        else:
            cluster = self.target_labels[index - self.source_length]
        return {"id": index, "source": feats, "domain": domain_index, 'cluster':cluster}
...
class BinarizedAudioDatasetMMDC(RawAudioDatasetMMDC):
    def __init__(
        self,
        data_dir,
        split,
