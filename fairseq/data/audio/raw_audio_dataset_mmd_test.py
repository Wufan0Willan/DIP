...
class RawAudioDatasetMMDTest(FairseqDataset):
    def __init__(
        self,
        sample_rate,
...
class FileAudioDatasetMMDTest(RawAudioDatasetMMDTest):
    def __init__(
        self,
        manifest_path,
        sample_rate,
        ...
        **mask_compute_kwargs,
        ...
        self.labels = []
        self.clusters = {"{0}".format(i):[] for i in range(self.num_clusters)}
        ...
        self.km_path = manifest_base+".km"
        with open(self.km_path, "r") as f:
            for i, label in enumerate(f):
                label = label.split("\n")[0]
                self.labels.append(label)
                self.clusters["{0}".format(label)].append(i)
        #pdb.set_trace()
        ...
        manifest_base = manifest_base.replace("valid", "test")        
        pdb.set_trace()
        ...
        with open("/home/wupeng/fairseq/sample_stat/test_source.tsv", "w") as fw:
        ...
        with open("/home/wupeng/fairseq/sample_stat/test_target.tsv", "w") as fw:
            for i in range(self.num_clusters):
    ...
    def __getitem__(self, index):
        ...
        #pdb.set_trace()
        src_label = self.labels[index]
        tgt_clusters = self.clusters[src_label] 
...
class BinarizedAudioDatasetMMDTest(RawAudioDatasetMMDTest):
    def __init__(
        self,
        data_dir,
        split,
