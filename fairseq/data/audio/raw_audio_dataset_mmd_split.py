...
class RawAudioDatasetMMDSplit(FairseqDataset):
    def __init__(
        self,
        sample_rate,
...
class FileAudioDatasetMMDSplit(RawAudioDatasetMMDSplit):
    def __init__(
        self,
        manifest_path,
        sample_rate,
        ...
        **mask_compute_kwargs,
        ...
        self.num_clusters = 1
        print("num of clusters:{0}".format(self.num_clusters))
        self.labels = []
        self.source_clusters = {"{0}".format(i):[] for i in range(self.num_clusters)}
        self.target_clusters = {"{0}".format(i):[] for i in range(self.num_clusters)}
        ...
        self.source_km_path = manifest_base+"_source.km"
        self.target_km_path = manifest_base+"_target.km"
        with open(self.source_km_path, "r") as f:
            for i, label in enumerate(f):
                label = label.split("\n")[0]
                self.labels.append([label,0])
                self.source_clusters["{0}".format(label)].append(i)
        with open(self.target_km_path, "r") as f:
            for i, label in enumerate(f):
                label = label.split("\n")[0]
                self.labels.append([label,1])
                self.target_clusters["{0}".format(label)].append(i)
        #pdb.set_trace()
    ...
    def __getitem__(self, index):
        ...
        src_label, domain = self.labels[index]
        assert domain == domain_index
        if target_domain_index == 0:
            tgt_clusters = self.source_clusters[src_label]
            if len(tgt_clusters) == 0:
                tgt_clusters = self.target_clusters[src_label]
        else:
            tgt_clusters = self.target_clusters[src_label]
            if len(tgt_clusters) == 0:
                tgt_clusters = self.source_clusters[src_label]
        tgt_index = np.random.choice(tgt_clusters, 1, replace=True)
        tgt_domain_index = self.domain_index[tgt_index]
        tgt_fn = self.fnames[tgt_index[0]]
        #print(tgt_index[0])
        tgt_fn = tgt_fn if isinstance(self.target_fnames, list) else tgt_fn.as_py()
        tgt_fn = self.text_compressor.decompress(tgt_fn)
        tgt_root_dir = self.target_root_dir if tgt_domain_index == 0 else self.target_root_dir
...
class BinarizedAudioDatasetMMDSplit(RawAudioDatasetMMDSplit):
    def __init__(
        self,
        data_dir,
        split,
