...
@register_criterion("wav2vec_mmd_full", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        losses.append((loss_list[0]+loss_list[1]))
        ...
        for lk in self.log_keys:
            ...
            elif lk in net_outputs[0] and lk in net_outputs[1]:
                lk_name = 'source_{0}'.format(lk)
                value = net_outputs[0][lk]
                if not is_xla_tensor(value):
                    value = float(value)
                logging_output[lk_name] = value

                lk_name = 'target_{0}'.format(lk)
                value = net_outputs[1][lk]
                if not is_xla_tensor(value):
                    value = float(value)
                logging_output[lk_name] = value
            ...
            with torch.no_grad():
                if logits.numel() == 0:
                    ...
                    for i,logits in enumerate(logits_list):
                        ...
                        logging_output["correct_{0}".format(i)] = corr
                        logging_output["count_{0}".format(i)] = count
