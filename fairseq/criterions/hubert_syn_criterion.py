...
@register_criterion("hubert_syn", dataclass=HubertCriterionConfig)
class HubertCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        """
        syn_waveform = model.synthesis_waveform(**sample["net_input"])
        #pdb.set_trace()
        #loss = self.loss_func(syn_waveform, sample["net_input"]["source"].unsqueeze(1))
        #sources = sample["net_input"]["source"].unsqueeze(1).float()
        #est_sources = syn_waveform.float()
        sources = sample["net_input"]["source"].unsqueeze(1)
        est_sources = syn_waveform
        device = sources.device
        loss = self.loss_func(est_sources, sources)
        print(loss)
        pdb.set_trace()
        sample_size = 0 
        logging_output = {
