...
@register_criterion("wav2vec_mmd", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.loss_weights is not None:
            ...
            extra_losses.append(net_output["mmd_loss"])
            #extra_losses.append(net_output["mmd_mean_loss"])
            #extra_losses.append(net_output["mmd_var_loss"])
            #pdb.set_trace()
        ...
        logging_output = {
            ...
            "mmd_loss": net_output["mmd_loss"].item() if (reduce and not self.xla) else net_output["mmd_loss"].detach(),
            #"mmd_mean_loss": net_output["mmd_mean_loss"].item() if (reduce and not self.xla) else net_output["mmd_mean_loss"].detach(),
            #"mmd_var_loss": net_output["mmd_var_loss"].item() if (reduce and not self.xla) else net_output["mmd_var_loss"].detach(),
            "ntokens": sample_size,
