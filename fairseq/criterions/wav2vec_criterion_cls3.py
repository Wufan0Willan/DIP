...
@register_criterion("wav2vec_mmd_cls3", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        src_loss, src_sample_size, src_logging_output = self.forward_cpc(model, src_output, sample, src_logits, src_target, src_cpc_loss, mmd_loss_f, mmd_loss_u, domain="src")
        tgt_loss, tgt_sample_size, tgt_logging_output = self.forward_cpc(model, tgt_output, sample, tgt_logits, tgt_target, tgt_cpc_loss, mmd_loss_f, mmd_loss_u, domain="tgt")
    ...
    def forward_logits(self, model, net_output, sample, xvec, reduce=True):
        ...
        if self.infonce:
            ...
            utt_nce_loss = F.cross_entropy(logits, target, reduction="none").view(-1, bsz)
            utt_nce_loss = torch.mean(utt_nce_loss, axis=0)
            weighted_xvec = torch.mul(torch.exp(-utt_nce_loss).unsqueeze(-1), xvec)
        else:
    ...
    def forward_cpc(self, model, net_output, sample, logits, target, loss, mmd_loss_u, mmd_loss_f, domain="src", reduce=True):
        ...
        return loss, sample_size, logging_output, utt_loss
