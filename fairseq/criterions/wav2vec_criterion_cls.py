...
@register_criterion("wav2vec_mmd_cls", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        src_output, tgt_output, mmd_loss_u, mmd_loss_f = model(**sample["net_input"])
        #pdb.set_trace()
        src_loss, src_sample_size, src_logging_output = self.forward_cpc(model, src_output, sample, mmd_loss_u, mmd_loss_f, domain="src")
        tgt_loss, tgt_sample_size, tgt_logging_output = self.forward_cpc(model, tgt_output, sample, mmd_loss_u, mmd_loss_f, domain="tgt")
    ...
    def forward_cpc(self, model, net_output, sample, mmd_loss_u, mmd_loss_f, domain="src", reduce=True):
        ...
        if self.loss_weights is not None:
            ...
            extra_losses.append(mmd_loss_u/2)
            extra_losses.append(mmd_loss_f/2)
