...
@register_criterion("wav2vecpit2", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        net_output, net_output_1, net_output_2 = model(**sample["net_input"])
        #pdb.set_trace()
        ...
        #pdb.set_trace()
        if self.infonce:
            logits_mix = torch.concat([logits_1.unsqueeze(0).unsqueeze(1), logits_2.unsqueeze(0).unsqueeze(1)], axis=1)
            targets_mix = torch.concat([target_1.unsqueeze(0).unsqueeze(1), target_2.unsqueeze(0).unsqueeze(1)], axis=1)
            loss_pit = self.pit_wrapper(logits_mix, targets_mix, reduction=reduction)
            loss_cpc = F.cross_entropy(logits, target, reduction=reduction) 
            loss = self.loss_ratio[0] * loss_cpc + self.loss_ratio[1] * loss_pit
        else:
        ...
        if self.loss_weights is not None:
            ...
            extra_losses_cpc = model.get_extra_losses(net_output)
