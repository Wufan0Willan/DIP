...
@register_criterion("wav2vecmix", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        net_output, net_output_mix = model(**sample["net_input"])
        #pdb.set_trace()
        logits = model.get_logits(net_output).float()
        target = model.get_targets(sample, net_output)

        logits_mix = model.get_logits(net_output_mix).float()
        target_mix = model.get_targets(sample, net_output_mix)
        ...
        if self.infonce:
            ...
            loss_mix = F.cross_entropy(logits_mix, target_mix, reduction=reduction)
            if self.loss_ratio:
                loss = self.loss_ratio[0] * loss_cpc + self.loss_ratio[1] * loss_mix
            else:
                loss = loss_cpc
        else:
