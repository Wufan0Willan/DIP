...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def forward(
        ...
        if features_only:
            ...
            #pdb.set_trace()
            #xvec = torch.concat([mean, var], axis=-1)
            return {
                "x": x,
                #"x": xvec,
                #"x": torch.mean(x, 1, keepdim=True),
                #"x": torch.var(x, 1, keepdim=True),
                #"x": torch.mean(proj_features, 1, keepdim=True),
...
class ConvFeatureExtractionModel(nn.Module):
    ...
    def get_imd_features(self, x, layer_index=100):
        x = x.unsqueeze(1)
        conv_rep_list = []
        ...
        for i,conv in enumerate(self.conv_layers):
            if i < layer_index:
                x = conv(x)
                conv_rep_list.append(x)
                #print(i)
                #print(x.size())
            else:
                break
        return x, conv_rep_list

    def gen_imd_features(self, x, conv_rep_list, layer_index=0):
        for i,conv in enumerate(self.conv_layers):
            if i < layer_index:
                continue
            else:
