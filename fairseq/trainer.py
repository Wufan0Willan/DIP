    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                #)
                #pdb.set_trace()
                #state["model"]["final_proj_1.weight"] = state["model"]["final_proj.weight"]
                #state["model"]["final_proj_1.bias"] = state["model"]["final_proj.bias"]
                #state["model"]["final_proj_2.weight"] = state["model"]["final_proj.weight"]  
                #state["model"]["final_proj_2.bias"] = state["model"]["final_proj.bias"]
                #print("init pred head from checkpoint")
                self.model.load_state_dict(
                    #state["model"], strict=True, model_cfg=self.cfg.model
